...
describe("fishnet", () => {
  ...
  it("Create an app (including a fee), an token to mint unlimited editions and buy some, checks payment data is correct, withdraw to check fee", async () => {
    ...
    let firstIdNumberArray = [...firstId];
    let secondIdNumberArray = [...secondId];
    await program.methods
      .createProduct(
        firstIdNumberArray,
        secondIdNumberArray,
    ...
    assert.equal(preBuyTokenAccount.firstId.toString(), firstIdNumberArray.toString());
    assert.equal(
    ...
    console.log(tokenConfig.toString(), paymentPublicKey.toString(), tokenMint.toString(), acceptedMintPublicKey.toString(), paymentVaultPublicKey.toString(), buyerTransferVault.toString(), buyerTokenVault.toString())
    await program.methods
